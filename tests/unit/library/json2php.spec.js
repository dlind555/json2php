...
describe("JsonToPhp", () => {
  it("Converts simple values", () => {
    let input = "true";
    expect(json2php(JSON.parse(input), true)).toBe(input);
  ...
  it("Converts numbers", () => {
    ...
    input = "123456789e+20";
    expect(json2php(JSON.parse(input), true)).toBe("1.23456789e+28");

    input = "0.000000005E-20";
    expect(json2php(JSON.parse(input), true)).toBe("5e-29");
  ...
  it("Converts an object with nested array in compact mode", () => {
    let input = '{"a":1,"b":[1,2]}';
    expect(json2php(JSON.parse(input), true)).toBe('["a" => 1,"b" => [1,2]]');
  });
